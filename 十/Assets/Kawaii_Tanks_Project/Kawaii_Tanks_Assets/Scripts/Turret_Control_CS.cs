...
namespace ChobiAssets.KTP
{
	...
	public class Turret_Control_CS : MonoBehaviour
	{
		...
		void LateUpdate ()
		{
            ...
            else if (idScript && !idScript.isPlayer)
            {
                Camera main = transform.parent.gameObject.GetComponentInChildren<Camera>();
                Cast_Ray_Sphere(main.transform.position, false);
            }
		}
        ...
        void FixedUpdate ()
		{
            ...
            else if (!idScript.isPlayer)
            {
                if (targetTransform)
                {
                    targetPos = targetTransform.position + (targetTransform.forward * targetOffset.z) + (targetTransform.right * targetOffset.x) + (targetTransform.up * targetOffset.y);
                    Fire_Control_CS fire = GetComponentInChildren<Fire_Control_CS>();
                    fire.onFiring();
                }
                else
                {
